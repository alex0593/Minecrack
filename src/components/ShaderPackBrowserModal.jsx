  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      if (source === 'modrinth') {
        ...
        let versions = await getProjectVersions(shader.project_id, {
          gameVersion: instance?.version,
        });
        // Fallback: si no hay versiones exactas, obtener la más reciente sin filtro
        if (!versions || versions.length === 0) {
          versions = await getProjectVersions(shader.project_id, {});
        }
        if (!versions || versions.length === 0) throw new Error('No se encontraron versiones para este shaderpack');
        ...
        let files = await getModFiles(shader.id, {
          gameVersion: instance?.version,
          limit: 5,
        });
        // Fallback sin filtro de versión
        if (!files || files.length === 0) {
          files = await getModFiles(shader.id, { limit: 5 });
        }
        if (!files || files.length === 0) throw new Error('No se encontraron archivos para este shaderpack');
