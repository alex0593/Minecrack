...
export default function VerifyInstanceModal({ instanceId, onClose }) {
  ...
  // Acciones de verificación: solo en pantalla de resultados (como antes)
  const footer = !loading && !repairing && verification ? (
    <>
      <button
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title="Verificar / Reparar"
      subtitle={instance.name}
      contentClassName="verify-modal"
      footer={footer}
    >
      {loading ? (
        <div style={{ padding: 'var(--gap-sm) var(--gap-md)', textAlign: 'center', color: 'var(--text-muted)' }}>
